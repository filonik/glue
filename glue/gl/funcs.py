...
_gltensortype_to_uniform_funcs = {
    ...
    (GL.GL_DOUBLE, (1,)): GL.glUniform1dv,
...
def vertex_attribute_disabler(gltype):
    ...
    if rank == 0:
        size = 1
        def disable_vertex_attrib_scalar(location):
            GL.glDisableVertexAttribArray(location)
        return disable_vertex_attrib_scalar
    elif rank == 1:
        size = gltype.dtype.sizes[0]
        def disable_vertex_attrib_vector(location):
            GL.glDisableVertexAttribArray(location)
        return disable_vertex_attrib_vector
    elif rank == 2:
        size = gltype.dtype.sizes[0]
        def disable_vertex_attrib_matrix(location):
            for n in range(size):
...
def attribute_setter(gltype, normalized=False, divisor=None):
    ...
    def attribute(location, value, type=None):
        ...
        if value is None:
            ...
            enable(location, stride=type.stride, offset=type.offset)
    return attribute